...
categories: [笔记, 开发]
tags: [笔记, 编程, 深度学习, 远程, 开发, Linux, CUDA]
...
若需避免重复输入，可执行`vim ~/.bashrc`将上述指令添加至末尾，实现代理配置的持久化，确保每次登录交互式Bash时自动启用代理环境；如果遇到持久配置的更新，使用`source ~/.bashrc`更新配置。
6. 验证验证代理是否生效：
...
1. **在shell中输入密码**：当尝试建立远程端口转发隧道时，有时会要求输入密码，与常规的密码输入不同，此时的密码是不可见的（连*都没有），凭印象输入后确认即可。
2. **协议前缀限制**：`https_proxy`的值不应错误输入为`https://127.0.0.1:7890`。原因是代理软件（如Clash）提供的HTTP代理服务本身是非加密的，若前缀写为`https`，客户端会尝试与代理端口进行SSL握手，导致握手失败并中断连接。
3. **权限报错**：若Windows侧出现`Bad owner or permissions on C:\\Users\[User]/.ssh/config`错误（发生在第4步），需进入该文件属性，在“安全-高级”中禁用权限继承，并将文件所有者更改为当前登录用户。有汇报见于[Issue](https://github.com/cmderdev/cmder/issues/1763)。
4. **V2ray**：V2ray包含HTTP和SOCKS5两套协议，其中HTTP默认使用端口10809，SOCKS5默认使用端口10808。
