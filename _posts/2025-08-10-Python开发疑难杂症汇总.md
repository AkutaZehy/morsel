---
title: Python开发疑难杂症汇总
date: 2025-08-10 12:00:00 +0800
categories: [笔记, 开发]
tags: [笔记, 编程, 总结, 深度学习, python, conda, linux, torch]
description: It's time to funk the code.
---

尽管在 [Conda 求解器故障实录：从 Solving Environment 到 libmamba]({% post_url 2025-07-22-Conda求解器故障实录：从SolvingEnvironment到libmamba %}) 一文中已经详细讨论了 Conda 的相关问题，然而，问题归问题，在现有项目中仍然需要继续使用，多少有些无奈。
...
一则实战参见[Mask2Former 多标签改造：第一周排障记录]({% post_url 2025-04-12-Mask2Former多标签改造：第一周排障记录 %})。
...
参见[Conda 开发两则：Conda 与 Pip 的协作边界]({% post_url 2025-07-16-Conda开发两则：Conda与Pip的协作边界 %})

### 我的Conda更新了，但是没更新？

参见[Conda 求解器故障实录：从 Solving Environment 到 libmamba]({% post_url 2025-07-22-Conda求解器故障实录：从SolvingEnvironment到libmamba %})

### setuptools 与 `pkg_resources` 的弃用与移除

> 官方迁移指南：[pkg_resources deprecation notice](https://setuptools.pypa.io/en/latest/pkg_resources.html)
{: .prompt-tip }

`pkg_resources` 是 `setuptools` 提供的一个用于包元数据查询、资源加载和命名空间包管理的模块。在早期 Python 打包生态中，它几乎是"事实标准"。然而，随着 Python 标准库的演进，`pkg_resources` 因其性能问题、设计缺陷以及与标准库功能的重复，逐渐被官方标记为弃用。

关于该工具，其更新历史可参加[该页面](https://setuptools.pypa.io/en/stable/history.html)，其关于 `pkg_resources` 的沿革历史如下：

| setuptools 版本 | 发布时间 | 关键变更 |
|----------------|----------|----------|
| **≥ 70.0.0** | 2024-07 | `pkg_resources` 正式标记为 **deprecated**，导入时触发 `DeprecationWarning` |
| **80.9.0** | 2025-05 | 官方设定移除截止日期：**2025-11-30** |
| **81.0.0** | 2026-02 | 移除 `pkg_resources` 的部分内部支持，部分场景下导入失败 |
| **82.0.0** | 2026-02 | `pkg_resources` **完全移除**，导入即报 `ModuleNotFoundError` |

由于该库近期的更新，在使用 clip / pyiqa 等依赖 `pkg_resources` 的旧库时容易出现典型报错：
...
如果项目依赖的第三方库尚未迁移，且你无法立即升级或替换它们，可以通过**降级 `setuptools`**（例如强制使用<81.0.0的版本） 临时恢复 `pkg_resources`。但 `pkg_resources` 的移除是 Python 打包生态现代化的一部分。官方推荐使用标准库替代方案：

| 原 `pkg_resources` 功能 | 推荐替代方案 |
|------------------------|--------------|
| `pkg_resources.get_distribution(name).version` | `importlib.metadata.version(name)` |
| `pkg_resources.resource_string(package, resource)` | `importlib.resources.files(package).joinpath(resource).read_bytes()` |
| `pkg_resources.iter_entry_points(group)` | `importlib.metadata.entry_points(group=group)` |
...
在Python 3.9，进一步引入[PEP 585 — Type Hinting Generics In Standard Collections](https://peps.python.org/pep-0585/)。让标准库的容器类和并发原语等原生支持泛型，避免依赖`typing.List`、`typing.Dict`这类“包装类”，`typing` 中的别名（如 `List`、`Dict`、`Tuple`）仍可用，但已不推荐在新代码中使用；标准库类（如 `list`、`dict`、`tuple`）通过实现 `__class_getitem__` 成为泛型类。
...
[LabelMe的安装说明（Windows）]({% post_url 2025-03-17-Labelme安装说明（Windows） %})
...
`compressai` 是 InterDigital 开源的一个基于 PyTorch 的端到端学习式图像/视频压缩库，提供熵模型（`EntropyBottleneck`、`GaussianConditional`）、常用网络层与算子，以及一个 C++ 实现的 rANS 熵编解码器（`compressai.ans`）。它本身不训练模型，更多是被各类学习式压缩项目当作底座依赖——例如 StableCodec / AEIC 的熵编码模块和辅助编码器 ELIC 都直接 import 它。

这个包的安装是一个显著的特例：**它在 PyPI 上只有源码包（sdist），没有预编译 wheel，而且它的构建依赖里写着 `torch`。**
...
```
⠓ updating lock-file   [00:29:55]  [━━━━━━━╾──────────]    1/3
  ⠈ default:linux-64    [00:29:41]  resolving compressai==1.2.6
```

半小时不动，而它自己只有一两百 KB、几乎全是 Python 代码，看起来完全不该卡。
...
第一层是 sdist 本身：没有 wheel 就意味着必须**现编译**。它的 `setup.py` 用 `Pybind11Extension` 编译 `compressai.ans` 这个 C++ 扩展（rANS 熵编解码器），需要 C++ 编译器和 pybind11。

第二层更隐蔽：构建依赖里带 `torch`，意味着解析器为了**读到它的依赖元数据**，就得先建一个隔离的构建环境、在里面装好 torch（几百 MB 到 2GB，取决于走哪个索引），再编译扩展。也就是说，仅仅为了"看一眼这个包依赖什么"，就要先拉一遍 torch——日志里 `resolving compressai==1.2.6` 卡住的正是这一步。

雪上加霜的是它常常是别人的传递依赖，删不干净：

```bash
$ pip show compressai
Required-by: neuralcompression
```

此时只删掉自己写的那行 `compressai = "==1.2.6"` 是没用的，解析器仍会顺着 `neuralcompression` 把它拉回来。若环境里还配了墙外索引（如 `download.pytorch.org`）并启用 `unsafe-best-match` 策略，每个包都要去该索引查一遍，卡顿会进一步放大。

解法是分两步，让它绕开"解析阶段就要装 torch"这一环：

```bash
# 1) 先让解析器装完其余依赖（配置里不要出现 compressai 及依赖它的包）
pixi install
# 2) 补齐构建工具，关掉构建隔离，直接用环境里已有的 torch 编译
pixi run pip install pybind11 setuptools wheel -i https://pypi.tuna.tsinghua.edu.cn/simple
pixi run pip install "compressai==1.2.6" --no-build-isolation -i https://pypi.tuna.tsinghua.edu.cn/simple
```

`--no-build-isolation` 是关键：它让 pip 直接复用当前环境里的 torch/pybind11，而不是另起一个要重新装 torch 的隔离环境；代价是这些构建依赖必须自己先装好。

> 如果那个引入 compressai 的上游包其实用不到，直接把它从依赖里去掉更省事。本例中 `neuralcompression` 只被项目自带的 `evaluate.py` 引用，而评测我们用的是另一套脚本。
{: .prompt-tip }
...
参见[Mask2Former 开发排障记录]({% post_url 2025-04-06-Mask2Former开发排障记录 %})
